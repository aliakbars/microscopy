...
def create_patches(img_basenames, annotation_dir, image_dir, size, step, grayscale=True, downsample=1, objectclass=None, negative_discard_rate=.9):
    ...
    neg = []
    s = 1
    for img_filename in tqdm(img_basenames):
        annotation_filename = annotation_dir + img_filename[:-3] + 'xml'
    ...
    np_patches = np.empty((len(patches),channels,int(size/downsample),int(size/downsample)),dtype=np.uint8)
